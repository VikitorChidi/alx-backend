...
const getCurrentAvailableSeatsAsync = promisify(redisClient.get).bind(
  redisClient
);
...
app.get("/process", async (_, res) => {
  ...
  queue.process("reserve_seat", async (job, done) => {
    const currentSeats = parseInt(
      await getCurrentAvailableSeatsAsync("available_seats"),
      10
    );
...
reserveSeatAsync("available_seats", MAX_SEATS)
  .then(() => {
